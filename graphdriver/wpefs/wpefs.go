...
func (devices *RbdSet) setupBaseImage() error {
	...
	log.Debugf("Create base wpefs image %s", baseName)
...
func (devices *RbdSet) imageIsMapped(devInfo *DevInfo) (bool, error) {
	// Older wpefs binaries are not printing the device on mapping so
	// we have to discover it with showmapped.
	out, err := exec.Command("wpefs", "showmapped", "--format", "json").Output()
	if err != nil {
		log.Errorf("Rbd run wpefs showmapped failed: %v", err)
		return false, err
	}
...
func (devices *RbdSet) mapImageToRbdDevice(devInfo *DevInfo) error {
	...
	_, err := exec.Command("wpefs", "--pool", pool, "map", imgName).Output()
	if err != nil {
		return err
	}
...
func (devices *RbdSet) unmapImageFromRbdDevice(devInfo *DevInfo) error {
	...
	if err := exec.Command("wpefs", "unmap", devInfo.Device).Run(); err != nil {
		return err
	}
...
func (devices *RbdSet) MountDevice(hash, mountPoint, mountLabel string) error {
	...
	if info.mountCount > 0 {
		if mountPoint != info.mountPath {
			return fmt.Errorf("Trying to mount wpefs device in multple places (%s, %s)", info.mountPath, info.Device)
		}
...
func NewRbdSet(root string, doInit bool, options []string) (*RbdSet, error) {
	...
	for _, option := range options {
		...
		case "wpefs.imageprefix":
			devices.imagePrefix = val
		case "wpefs.client":
			devices.clientId = val
		case "wpefs.configfile":
